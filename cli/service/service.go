...
import (
	"context"
	"fmt"
	pb "github.com/noah-blockchain/noah-go-node/cli/cli_pb"
	"github.com/noah-blockchain/noah-go-node/config"
	"github.com/noah-blockchain/noah-go-node/core/noah"
	"github.com/noah-blockchain/mintnoaher-go-node/core/types"
...
func NewManager(blockchain *noah.Blockchain, tmRPC *rpc.Local, tmNode *tmNode.Node, cfg *config.Config) pb.ManagerServiceServer {
...
func (m *Manager) Dashboard(_ *empty.Empty, stream pb.ManagerService_DashboardServer) error {
	for {
		...
		case <-time.After(time.Second):
			...
			switch true {
			case validator != nil && validatorStatus == noah.ValidatorAbsent:
				pbValidatorStatus = pb.DashboardResponse_Validating
			case validator == nil && validatorStatus == noah.ValidatorAbsent:
				pbValidatorStatus = pb.DashboardResponse_Challenger
			case validator == nil && validatorStatus == noah.ValidatorPresent:
