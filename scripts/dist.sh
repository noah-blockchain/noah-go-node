...
CGO_ENABLED=1 CGO_LDFLAGS="-lsnappy" go build -tags "noah gcc" -ldflags "-s -w -X ${GIT_IMPORT}.GitCommit=${GIT_COMMIT}" -o "build/pkg/darwin_amd64/noah" ./cmd/noah

echo "==> Building for linux in docker"
docker run -t -v ${PWD}:/go/src/github.com/noah-blockchain/noah-go-node/ -i 6a38838f45ef sh -c 'CGO_ENABLED=1 CGO_LDFLAGS="-lsnappy" go build -tags "noah gcc" -ldflags "-s -w -X ${GIT_IMPORT}.GitCommit=${GIT_COMMIT}" -o "build/pkg/linux_amd64/noah" ./cmd/noah/'
