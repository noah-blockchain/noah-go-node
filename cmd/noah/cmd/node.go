...
func runNode() error {
	...
	if err := common.EnsureDir(fmt.Sprintf("%s/config", utils.GetNoahHome()), 0777); err != nil {
		return err
	}

	if err := common.EnsureDir(fmt.Sprintf("%s/tmdata", utils.GetNoahHome()), 0777); err != nil {
		return err
	}
...
func getGenesis() (doc *tmTypes.GenesisDoc, e error) {
	genesisFile := fmt.Sprintf("%s/config/genesis.json", utils.GetNoahHome())
