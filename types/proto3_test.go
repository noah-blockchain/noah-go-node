...
import (
	proto32 "github.com/noah-blockchain/noah-go-node/types/proto3"
	"testing"
	"time"
...
func TestProto3Compatibility(t *testing.T) {
	...
	pbHeader := proto32.Header{
		ChainID: "cosmos",
		Height:  150,
		Time:    &proto32.Timestamp{Seconds: seconds, Nanos: nanos},
		NumTxs:  7,
		LastBlockID: &proto32.BlockID{
			Hash: []byte("some serious hashing"),
			PartsHeader: &proto32.PartSetHeader{
				Total: 8,
	...
	emptyLastBlockPb := proto32.Header{
		ChainID:        "cosmos",
		Height:         150,
		Time:           &proto32.Timestamp{Seconds: seconds, Nanos: nanos},
		NumTxs:         7,
	...
	pb, err = proto.Marshal(&proto32.Header{})
	assert.NoError(t, err, "unexpected error")
	t.Log(pb)
	...
	pb, err = proto.Marshal(&proto32.Timestamp{})
	assert.NoError(t, err, "unexpected error")
	t.Log(pb)
