...
func TestBlockValidateBasic(t *testing.T) {
	...
	testCases := []struct {
		...
		{"Tampered Data", func(blk *Block) {
			Data.Txs[0] = Tx("something else")
			Data.hash = nil // clear hash or change wont be noticed
		}, true},
...
func TestSignedHeaderValidateBasic(t *testing.T) {
	...
	testCases := []struct {
		...
		{"Valid Signed Header", Header, validSignedHeader.Commit, false},
		{"Invalid Signed Header", Header, validSignedHeader.Commit, true},
		{"Invalid Signed Header", Header, invalidSignedHeader.Commit, true},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.testName, func(t *testing.T) {
			...
			assert.Equal(t, tc.expectErr, sh.ValidateBasic(Header.ChainID) != nil, "Validate Basic had an unexpected result")
		})
	}
}
