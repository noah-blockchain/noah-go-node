...
import (
	...
	counter2 "github.com/noah-blockchain/noah-go-node/abci/example/counter"
	...
	"github.com/noah-blockchain/noah-go-node/abci/tests/server"
	types2 "github.com/noah-blockchain/noah-go-node/abci/types"
	version2 "github.com/noah-blockchain/noah-go-node/abci/version"
	"io"
	"os"
	"strings"
...
var RootCmd = &cobra.Command{
	...
	PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
		...
		if client == nil {
			var err error
			client, err = abcicli2.NewClient(flagAddress, flagAbci, false)
			if err != nil {
				return err
			}
...
func cmdTest(cmd *cobra.Command, args []string) error {
	return compose(
		[]func() error{
			func() error { return testsuite.InitChain(client) },
			func() error { return testsuite.SetOption(client, "serial", "on") },
			func() error { return testsuite.Commit(client, nil) },
			func() error { return testsuite.DeliverTx(client, []byte("abc"), code2.CodeTypeBadNonce, nil) },
			func() error { return testsuite.Commit(client, nil) },
			func() error { return testsuite.DeliverTx(client, []byte{0x00}, code2.CodeTypeOK, nil) },
			func() error { return testsuite.Commit(client, []byte{0, 0, 0, 0, 0, 0, 0, 1}) },
			func() error { return testsuite.DeliverTx(client, []byte{0x00}, code2.CodeTypeBadNonce, nil) },
			func() error { return testsuite.DeliverTx(client, []byte{0x01}, code2.CodeTypeOK, nil) },
			func() error { return testsuite.DeliverTx(client, []byte{0x00, 0x02}, code2.CodeTypeOK, nil) },
			func() error { return testsuite.DeliverTx(client, []byte{0x00, 0x03}, code2.CodeTypeOK, nil) },
			func() error { return testsuite.DeliverTx(client, []byte{0x00, 0x00, 0x04}, code2.CodeTypeOK, nil) },
			func() error {
				return testsuite.DeliverTx(client, []byte{0x00, 0x00, 0x06}, code2.CodeTypeBadNonce, nil)
			},
			func() error { return testsuite.Commit(client, []byte{0, 0, 0, 0, 0, 0, 0, 5}) },
		})
}
...
func cmdInfo(cmd *cobra.Command, args []string) error {
	...
	res, err := client.InfoSync(types2.RequestInfo{Version: version})
	if err != nil {
		return err
	}
	printResponse(cmd, args, response{
...
func cmdSetOption(cmd *cobra.Command, args []string) error {
	...
	_, err := client.SetOptionSync(types2.RequestSetOption{Key: key, Value: val})
	if err != nil {
		return err
	}
...
func cmdCheckTx(cmd *cobra.Command, args []string) error {
	...
	res, err := client.CheckTxSync(types2.RequestCheckTx{Tx: txBytes})
	if err != nil {
		return err
	}
	printResponse(cmd, args, response{
...
func cmdQuery(cmd *cobra.Command, args []string) error {
	...
	resQuery, err := client.QuerySync(types2.RequestQuery{
		Data:   queryBytes,
		Path:   flagPath,
...
func cmdCounter(cmd *cobra.Command, args []string) error {
	app := counter2.NewCounterApplication(flagSerial)
	logger := log.NewTMLogger(log.NewSyncWriter(os.Stdout))

	// Start the listener
	srv, err := server2.NewServer(flagAddress, flagAbci, app)
	if err != nil {
		return err
	}
...
func cmdKVStore(cmd *cobra.Command, args []string) error {
	...
	var app types2.Application
	if flagPersist == "" {
		app = kvstore2.NewKVStoreApplication()
	} else {
		app = kvstore2.NewPersistentKVStoreApplication(flagPersist)
		app.(*kvstore2.PersistentKVStoreApplication).SetLogger(logger.With("module", "kvstore"))
	}
