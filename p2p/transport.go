...
type peerConfig struct {
	chDescs     []*conn2.ChannelDescriptor
...
func (mt *MultiplexTransport) acceptPeers() {
	for {
		...
		go func(c net.Conn) {
			var (
				nodeInfo   NodeInfo
				secretConn *conn2.SecretConnection
				netAddr    *NetAddress
			)
...
) (secretConn *conn2.SecretConnection, nodeInfo NodeInfo, err error) {
	defer func() {
		if err != nil {
			_ = mt.cleanup(c)
		}
...
) (*conn2.SecretConnection, error) {
	if err := c.SetDeadline(time.Now().Add(timeout)); err != nil {
		return nil, err
	}

	sc, err := conn2.MakeSecretConnection(c, privKey)
	if err != nil {
		return nil, err
	}
