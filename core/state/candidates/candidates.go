...
func (c *Candidates) RecalculateStakes(height uint64) {
	for _, candidate := range c.list {
		stakes := c.GetStakes(candidate.PubKey)
		for _, stake := range stakes {
			stake.setNoahValue(c.calculateNoahValue(stake.Coin, stake.Value, false, true))
		}
		...
		for _, update := range updates {
			update.setNoahValue(c.calculateNoahValue(update.Coin, update.Value, false, true))
		}
...
func (c *Candidates) IsNewCandidateStakeSufficient(coin types.CoinSymbol, stake *big.Int) bool {
	noahValue := c.calculateNoahValue(coin, stake, true, true)
	candidates := c.list

	for _, candidate := range candidates {
		if candidate.totalNoahValue.Cmp(noahValue) == -1 {
			return true
		}
	}
...
func (c *Candidates) calculateNoahValue(coinSymbol types.CoinSymbol, amount *big.Int, includeSelf, includeUpdates bool) *big.Int {
