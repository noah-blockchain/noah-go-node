...
func TestStateExport(t *testing.T) {
	...
	state.Coins.Create(
		coinTest,
		"TEST",
		helpers.NoahToQNoah(big.NewInt(1)),
		10,
	...
	state.Coins.Create(
		coinTest2,
		"TEST2",
		helpers.NoahToQNoah(big.NewInt(2)),
		50,
	...
	state.Accounts.AddBalance(address1, types.GetBaseCoin(), helpers.NoahToQNoah(big.NewInt(1)))
	state.Accounts.AddBalance(address1, coinTest, helpers.NoahToQNoah(big.NewInt(1)))
	state.Accounts.AddBalance(address2, coinTest2, helpers.NoahToQNoah(big.NewInt(2)))
	...
	state.FrozenFunds.AddFund(height, address1, candidatePubKey1, coinTest, helpers.NoahToQNoah(big.NewInt(100)))
	state.FrozenFunds.AddFund(height+10, address1, candidatePubKey1, types.GetBaseCoin(), helpers.NoahToQNoah(big.NewInt(3)))
	state.FrozenFunds.AddFund(height+100, address2, candidatePubKey1, coinTest, helpers.NoahToQNoah(big.NewInt(500)))
	state.FrozenFunds.AddFund(height+150, address2, candidatePubKey1, coinTest2, helpers.NoahToQNoah(big.NewInt(1000)))
	...
	if newStateCoin.Name != "TEST" ||
		...
		newStateCoin.Volume != helpers.NoahToQNoah(big.NewInt(1)).String() ||
	...
	if newStateCoin1.Name != "TEST2" ||
		...
		newStateCoin1.Volume != helpers.NoahToQNoah(big.NewInt(2)).String() ||
	...
	if account1.Balance[0].Coin != coinTest || account1.Balance[0].Value != helpers.NoahToQNoah(big.NewInt(1)).String() {
		t.Fatal("Wrong new state account balance data")
	}

	if account1.Balance[1].Coin != types.GetBaseCoin() || account1.Balance[1].Value != helpers.NoahToQNoah(big.NewInt(1)).String() {
		t.Fatal("Wrong new state account balance data")
	}

	if account2.Balance[0].Coin != coinTest2 || account2.Balance[0].Value != helpers.NoahToQNoah(big.NewInt(2)).String() {
		t.Fatal("Wrong new state account balance data")
	}
