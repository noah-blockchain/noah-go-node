...
import (
	...
	"github.com/noah-blockchain/noah-go-node/legacy/candidates"
...
func getTestGenesis(pv *privval.FilePV) func() (*types2.GenesisDoc, error) {
	return func() (*types2.GenesisDoc, error) {
		...
		validators, candidates := makeTestValidatorsAndCandidates([]string{base64.StdEncoding.EncodeToString(pv.Key.PubKey.Bytes()[5:])}, helpers.NoahToQNoah(big.NewInt(1000)))
...
func TestBlockchain_GetStateForHeightAndDeleteStateVersions(t *testing.T) {
	...
	data := transaction.CreateCoinData{
		Name:                 "nAAA123",
		Symbol:               symbol,
		InitialAmount:        helpers.NoahToQNoah(big.NewInt(1000000)),
		InitialReserve:       helpers.NoahToQNoah(big.NewInt(10000)),
...
func TestBlockchain_FrozenFunds(t *testing.T) {
	...
	value := helpers.NoahToQNoah(big.NewInt(1000))
	pubkey := types.BytesToPubkey(pv.Key.PubKey.Bytes()[5:])
...
func TestBlockchain_RecalculateStakes_andRemoveValidator(t *testing.T) {
	...
	blockchain.lock.RUnlock()
	{
		buyCoinData := transaction.BuyCoinData{
			CoinToBuy:          coinID,
			ValueToBuy:         helpers.NoahToQNoah(big.NewInt(10000000)),
			CoinToSell:         0,
			MaximumValueToSell: helpers.NoahToQNoah(big.NewInt(10000000000000000)),
		}
	...
	nonce++
	{
		delegateData := transaction.DelegateData{
			PubKey: types.BytesToPubkey(pv.Key.PubKey.Bytes()[5:]),
			Coin:   coinID,
			Value:  helpers.NoahToQNoah(big.NewInt(9000000)),
		}
...
func TestStopNetworkByHaltBlocks(t *testing.T) {
	...
	blockchain.stateDeliver.Validators.Create(v1Pubkey, helpers.NoahToQNoah(big.NewInt(3)))
	blockchain.stateDeliver.Validators.Create(v2Pubkey, helpers.NoahToQNoah(big.NewInt(5)))
	blockchain.stateDeliver.Validators.Create(v3Pubkey, helpers.NoahToQNoah(big.NewInt(3)))
