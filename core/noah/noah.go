...
// Blockchain is a main structure of noah
...
// NewNoahBlockchain creates noah Blockchain instance, should be only called once
...
// Info return application info. Used for synchronization between Tendermint and Noah
...
// Stop gracefully stopping Noah Blockchain instance
...
// CurrentState returns immutable state of Noah Blockchain
...
// GetStateForHeight returns immutable state of Noah Blockchain for given height
...
// Height returns current height of Noah Blockchain
		...
		for _, val := range vals {
			...
			for _, halt := range halts.List {
				if halt.Pubkey == val.PubKey {
					totalVotedPower.Add(totalVotedPower, val.GetTotalNoahStake())
				}
			}
