...
func TestDeclareCandidacyTxOverflow(t *testing.T) {
	...
	for i := 0; i < maxCandidatesCount; i++ {
		...
		cState.Candidates.Delegate(types.Address{}, pubkey, types.GetBaseCoinID(), helpers.NoahToQNoah(big.NewInt(10)), helpers.NoahToQNoah(big.NewInt(10)))
	}
...
func TestDeclareCandidacyTxToGasCoinReserveUnderflow(t *testing.T) {
	...
	cState.Coins.AddVolume(coin, helpers.NoahToQNoah(big.NewInt(1000000)))
	cState.Accounts.AddBalance(addr, coin, helpers.NoahToQNoah(big.NewInt(1000000)))
	cState.Coins.SubReserve(coin, helpers.NoahToQNoah(big.NewInt(90000)))
	cState.Commit()
