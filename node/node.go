...
import (
	...
	"github.com/noah-blockchain/noah-go-node/evidence"
	"github.com/noah-blockchain/noah-go-node/state"
	...
	null2 "github.com/noah-blockchain/noah-go-node/state/txindex/null"
	"net"
	"net/http"
	_ "net/http/pprof"
	"os"
	...
	mempl "github.com/noah-blockchain/noah-go-node/mempool"
	"github.com/noah-blockchain/noah-go-node/p2p"
	"github.com/noah-blockchain/noah-go-node/p2p/pex"
	"github.com/noah-blockchain/noah-go-node/store"
	"github.com/noah-blockchain/noah-go-node/types"
	tmtime "github.com/noah-blockchain/noah-go-node/types/time"
...
type Node struct {
	...
	txIndexer        txindex2.TxIndexer
	indexerService   *txindex2.IndexerService
...
func createAndStartIndexerService(config *cfg.Config, dbProvider DBProvider,
	eventBus *types.EventBus, logger log.Logger) (*txindex2.IndexerService, txindex2.TxIndexer, error) {

	var txIndexer txindex2.TxIndexer
	...
	case "kv":
		...
		case config.TxIndex.IndexTags != "":
			txIndexer = kv2.NewTxIndex(store, kv2.IndexTags(splitAndTrimEmpty(config.TxIndex.IndexTags, ",", " ")))
		case config.TxIndex.IndexAllTags:
			txIndexer = kv2.NewTxIndex(store, kv2.IndexAllTags())
		default:
			txIndexer = kv2.NewTxIndex(store)
		}
	default:
		txIndexer = &null2.TxIndex{}
	}

	indexerService := txindex2.NewIndexerService(txIndexer, eventBus)
...
func makeNodeInfo(
	...
	txIndexerStatus := "on"
	if _, ok := txIndexer.(*null2.TxIndex); ok {
		txIndexerStatus = "off"
	}
